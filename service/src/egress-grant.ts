...
function validateGrant(value: unknown, token: string): EgressGrantClaims {
  ...
  if (claims.max_requests === undefined) {
    ...
    if (hasNetworkPolicy !== hasNetworkPolicyDigest) {
        throw new EgressGrantError(
            'malformed',
            'Egress network policy binding is incomplete',
        );
    }
    if (hasNetworkPolicy && hasNetworkPolicyDigest) {
        if (
            ...
            throw new EgressGrantError(
                'malformed',
                'Egress network policy digest is invalid',
            );
        }
        try {
            ...
            throw new EgressGrantError(
                'scope_mismatch',
                'Egress network policy binding is invalid',
            );
    }
  }
...
export function sealEgressGrant(
    ...
    ...(claims.service_id ? { service_id: claims.service_id } : {}),
            ...
            ...(claims.auth_context_hash
                ...
                : {}),
            ...(claims.network_policy
                ? { network_policy: claims.network_policy }
                : {}),
            ...(claims.network_policy_digest
                ? { network_policy_digest: claims.network_policy_digest }
                : {}),
        },
        secret,
    );
...
export function prepareSandboxEgress(args: {
  ...
  delete (sandboxVisibleClaims as unknown as Record<string, unknown>)['chc_user_id']; // leak-check:allow
