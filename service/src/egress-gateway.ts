...
type ExternalFetchAuditFields = Pick<
    EgressAuditFields,
  | 'execHash'
  | 'tenantHash'
  | 'userHash'
...
function routeFamily(req: Request): string {
  ...
  if (req.path === '/https-passthrough') return 'https-passthrough';
    if (req.path === '/package-transport') return 'package-transport';
  if (req.path.startsWith('/sessions/')) {
    if (req.method === 'PUT') return 'file-upload';
        if (req.method === 'GET' && req.path.includes('/objects/'))
            return 'file-download';
        if (req.method === 'GET' && req.path.endsWith('/objects'))
            return 'file-list';
    return 'file-unknown';
  }
  return 'unknown';
...
function isSyntheticEgressRequest(res: Response): boolean {
    return (
        res.locals.syntheticInternalRequest === true ||
...
function setPtcAudit(
    res: Response,
    args: { callbackExecId: string; requestExecId: string },
): void {
  res.locals.egressAuditFields = {
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  const id = req.header('x-request-id') ?? crypto.randomUUID();
    res.locals.syntheticInternalRequest =
        req.path.startsWith('/internal/') &&
        isSyntheticInternalRequestHeader(
            req.header(CODEAPI_SYNTHETIC_INTERNAL_REQUEST_HEADER),
        );
...
function errorStatus(error: EgressGrantError): number {
    if (error.reason === 'missing_secret' || error.reason === 'weak_secret')
        return 500;
  ...
  if (controlledEgressRoute(req)) {
    logger.error('Controlled egress request failed', {
      ...
      ...externalFetchAuditFields(res),
        ...
        return res
            .status(500)
            .json({ error: 'FETCH_FAILED', message: 'External fetch failed' });
  }
...
async function getGrant(
  ...
  if (!token) {
        throw new EgressGrantError(
            'malformed',
            `${EGRESS_GRANT_HEADER} is required`,
        );
  }
...
function decodeOriginalFilename(raw: string): string {
  try {
    return decodeURIComponent(raw);
        ...
        throw new EgressGrantError(
            'malformed',
            'X-Original-Filename is not valid percent-encoding',
        );
  }
}

function normalizePtcCallbackTimeoutSeconds(
    rawTimeoutSeconds: unknown,
): number {
    const raw =
        rawTimeoutSeconds === undefined
            ? env.EGRESS_GRANT_TTL_SECONDS
            : rawTimeoutSeconds;
...
function assertOutputFilenameAllowed(name: string): void {
  if (!name || name === '.') {
        throw new EgressGrantError(
            'malformed',
            'Output filename must not be empty',
        );
  }
    if (
        name.includes('\0') ||
        ...
        throw new EgressGrantError(
            'malformed',
            'Output filename must be a relative POSIX path',
        );
  }
  if (name.length > env.EGRESS_GATEWAY_MAX_PATH_LENGTH) {
        throw new EgressGrantError(
            'malformed',
            'Output filename exceeds gateway path length limit',
        );
  }
  ...
  if (parts.some(part => part === '..' || part === '.')) {
        throw new EgressGrantError(
            'malformed',
            'Output filename must not contain traversal segments',
        );
  }
  ...
  if (depth > env.EGRESS_GATEWAY_MAX_NESTING_DEPTH) {
        throw new EgressGrantError(
            'malformed',
            'Output filename exceeds gateway nesting depth limit',
        );
  }
  if (path.posix.normalize(name) !== name || name.endsWith('/')) {
        throw new EgressGrantError(
            'malformed',
            'Output filename must be canonical',
        );
  }
      ...
      SUPPORTED_OUTPUT_EXTENSIONS.has(basename.toLowerCase()) ||
            (ext === '' &&
                (SUPPORTED_OUTPUT_EXTENSIONS.has(dottedBasename) ||
                    SUPPORTED_OUTPUT_EXTENSIONS.has(
                        dottedBasename.toLowerCase(),
                    )));
    if (!allowed) {
            throw new EgressGrantError(
                'scope_mismatch',
                'Output filename extension is not supported',
            );
    }
  }
}

function inputFileKey(args: {
    session_id: string;
    id: string;
    name: string;
}): string {
...
function assertGrantSession(
  ...
  if (direction === 'read') {
    if (!grant.read_sessions.includes(sessionId)) {
            throw new EgressGrantError(
                'scope_mismatch',
                'Read session is outside the egress grant scope',
            );
    }
    return;
  }
  if (grant.output_session_id !== sessionId) {
        throw new EgressGrantError(
            'scope_mismatch',
            'Write session is outside the egress grant scope',
        );
  }
}

function openSessionParam(
    raw: string,
    grant: EgressGrantClaims,
    direction: 'read' | 'write',
): string {
  ...
  if (handle.typ !== 'session' || handle.dir !== direction) {
        throw new EgressGrantError(
            'wrong_type',
            `Expected an egress ${direction} session handle`,
        );
  }
  if (handle.exec_id !== grant.exec_id) {
        throw new EgressGrantError(
            'scope_mismatch',
            'Session handle execution does not match grant',
        );
  }
    if (
        env.EGRESS_LEDGER_REQUIRED &&
        handle.grant_id !== grant.grant_id &&
        !(grant.legacy_grant && handle.grant_id === undefined)
    ) {
        throw new EgressGrantError(
            'scope_mismatch',
            'Session handle grant does not match request grant',
        );
  }
...
function isAllowedReadObject(
  ...
  if (allowedFiles.has(inputFileKey(args))) return true;
    return (
        isDirkeepName(args.name) &&
        allowedDirkeeps.has(`${args.session_id}\0${args.name}`)
    );
}

function openObjectParam(
    raw: string,
    grant: EgressGrantClaims,
    sessionId: string,
  ...
  if (handle.exec_id !== grant.exec_id || handle.session_id !== sessionId) {
        throw new EgressGrantError(
            'scope_mismatch',
            'Object handle does not match grant/session',
        );
  }
    if (
        env.EGRESS_LEDGER_REQUIRED &&
        ...
        throw new EgressGrantError(
            'scope_mismatch',
            'Object handle grant does not match request grant',
        );
  }
  ...
  if (!allowed) {
        throw new EgressGrantError(
            'scope_mismatch',
            'Object handle is outside the egress grant file scope',
        );
  }
...
function declaredPassthroughBytes(
...
function configuredExternalFetchPolicy(): ExternalFetchPolicy {
    if (
        !cachedExternalFetchPolicy ||
        cachedExternalFetchPolicyPath !== env.EXTERNAL_FETCH_POLICY_FILE
    ) {
        cachedExternalFetchPolicy = loadExternalFetchPolicy(
...
function effectivePolicyForGrant(
    grant: EgressGrantClaims,
): ExternalFetchPolicy {
    return effectiveExternalFetchPolicy(
        grant.network_policy,
        grant.network_policy_digest,
...
async function readExternalFetchEnvelope(req: Request): Promise<string> {
  ...
  const parsedLength = parseBoundedContentLength(
    ...
    if (!parsedLength.ok)
        throw new ExternalFetchEnvelopeError(parsedLength.status);
  const body = await readRequestBody(req);
    if (body.length !== parsedLength.length)
        throw new ExternalFetchEnvelopeError();
  let parsed: unknown;
  try {
...
async function readHttpsPassthroughEnvelope(
    req: Request,
): Promise<HttpsPassthroughEnvelope> {
    if (req.headers['transfer-encoding'] !== undefined)
        throw new ExternalFetchEnvelopeError();
    if (
        (req.header('content-type') ?? '')
...
type PackageTransportEnvelope = {
    url: string;
    method: 'GET' | 'HEAD';
    headers: Record<string, string>;
};

async function readPackageTransportEnvelope(
    req: Request,
): Promise<PackageTransportEnvelope> {
    if (req.headers['transfer-encoding'] !== undefined)
        throw new ExternalFetchEnvelopeError();
    if (
        (req.header('content-type') ?? '')
    ...
    const parsedLength = parseBoundedContentLength(
        req.header('content-length'),
        65_536,
        'Package transport envelope exceeds limit',
    );
    if (!parsedLength.ok)
        throw new ExternalFetchEnvelopeError(parsedLength.status);
    const body = await readRequestBody(req);
    ...
    const raw = parsed as Record<string, unknown>;
    if (Object.keys(raw).sort().join(',') !== 'headers,method,url') {
        throw new ExternalFetchEnvelopeError();
    }
    if (
        typeof raw.url !== 'string' ||
        (raw.method !== 'GET' && raw.method !== 'HEAD') ||
        !raw.headers ||
        typeof raw.headers !== 'object' ||
        Array.isArray(raw.headers)
    ) {
    ...
    const rawHeaders = raw.headers as Record<string, unknown>;
    if (Object.keys(rawHeaders).length > 64)
        throw new ExternalFetchEnvelopeError(413);
    const headers: Record<string, string> = {};
    let headerBytes = 0;
    for (const [name, value] of Object.entries(rawHeaders)) {
        ...
        headerBytes += Buffer.byteLength(name) + Buffer.byteLength(value);
        if (headerBytes > 32_768) throw new ExternalFetchEnvelopeError(413);
        headers[lower] = value;
    }
    return { url: raw.url, method: raw.method, headers };
}

const PACKAGE_RESPONSE_HEADERS: Record<string, true> = {
    'accept-ranges': true,
    'cache-control': true,
...
function setPackageTransportResponseHeaders(
    headers: Record<string, string | string[] | undefined>,
    res: Response,
): void {
    for (const [name, value] of Object.entries(headers)) {
        if (
            value === undefined ||
            PACKAGE_RESPONSE_HEADERS[name.toLowerCase()] !== true
        )
            continue;
        res.setHeader(name, value);
    }
...
app.post(
    '/internal/egress-grants',
    express.json({ limit: env.HTTP_JSON_LIMIT }),
    requireConfiguredInternalServiceAuth,
    async (req, res) => {
  try {
...
async function restoreInternalSandboxResult(args: {
  ...
  if (args.expectedGrantId && grant.grant_id !== args.expectedGrantId) {
        throw new EgressGrantError(
            'scope_mismatch',
            'Restore grant id does not match token',
        );
  }
  if (grant.legacy_grant) {
    await ensureEgressLedger(grant);
  ...
  try {
    ...
    if (!body?.result || typeof body.egressGrantToken !== 'string') {
                return res
                    .status(400)
                    .json({
                        error: 'result and egressGrantToken are required',
                    });
    }
...
app.post(
    '/internal/egress-grants/:grantId/restore-result',
    express.json({ limit: env.HTTP_JSON_LIMIT }),
    requireConfiguredInternalServiceAuth,
    async (req, res) => {
  try {
...
app.post(
  ...
  try {
            const body = req.body as {
                reason?: unknown;
                egressGrantToken?: unknown;
            };
    if (typeof body?.egressGrantToken !== 'string') {
                return res
                    .status(400)
                    .json({ error: 'egressGrantToken is required' });
    }
            const grant = openEgressGrant(
                body.egressGrantToken,
                env.EGRESS_GRANT_SECRET,
            );
    setGrantAudit(res, grant);
    if (grant.legacy_grant) {
      await ensureEgressLedger(grant);
            ...
            const reason =
                typeof body.reason === 'string' ? body.reason : 'completed';
...
app.post(
    '/internal/egress-grants/:grantId/revoke',
  ...
  try {
    ...
    if (!body.executionId || !body.sessionId || !body.callbackToken) {
                return res
                    .status(400)
                    .json({
                        error: 'executionId, sessionId, and callbackToken are required',
                    });
    }
    const issuedAt = Math.floor(Date.now() / 1000);
            const timeoutSeconds = normalizePtcCallbackTimeoutSeconds(
                body.timeoutSeconds,
            );
            const expiresAt =
                issuedAt +
                Math.min(timeoutSeconds, env.EGRESS_GRANT_TTL_SECONDS);
    const grantId = nanoid();
    ...
    await createEgressLedger({
      ...
      typ: 'grant',
            ...
            return res
                .status(201)
                .json({ grant_id: grantId, callbackToken: token });
  } catch (error) {
    return sendEgressError(req, res, error);
  }
    },
...
app.get('/sessions/:sessionHandle/objects', async (req, res) => {
  try {
        if (
            Object.keys(req.query).some(key => key !== 'detail') ||
            req.query.detail !== 'normalized'
        ) {
            return res
                .status(400)
                .json({
                    error: 'Only detail=normalized object listings are supported',
                });
    }
    const grant = await getGrant(req, res);
        const sessionId = openSessionParam(
            req.params.sessionHandle,
            grant,
            'read',
        );
        ...
        '?detail=normalized',
      ),
            {
                headers: injectTraceHeaders(
                    internalServiceHeaders({ Accept: 'application/json' }),
                ),
            },
    );
    if (!upstream.ok) {
    ...
    if (!Array.isArray(data)) {
            return res
                .status(502)
                .json({ error: 'Invalid file-server object listing' });
    }
    ...
    const normalized = data
        ...
        typeof (obj as { name?: unknown }).name === 'string' &&
                    typeof (obj as { storage_session_id?: unknown })
                        .storage_session_id === 'string' &&
                    (obj as { storage_session_id: string })
  ...
  try {
    if (Object.keys(req.query).length > 0) {
            return res
                .status(400)
                .json({
                    error: 'Object download query parameters are not supported',
                });
    }
    const grant = await getGrant(req, res);
        const sessionId = openSessionParam(
        ...
        const object = openObjectParam(
            req.params.objectHandle,
            grant,
            sessionId,
        );
    await recordEgressRead(grant);
...
app.put('/sessions/:sessionHandle/objects/:fileId', async (req, res) => {
    let reservedUpload:
        | { grant: EgressGrantClaims; fileId: string; bytes: number }
        | undefined;
  try {
    if (Object.keys(req.query).length > 0) {
            return res
                .status(400)
                .json({
                    error: 'Object upload query parameters are not supported',
                });
    }
    const grant = await getGrant(req, res);
        const sessionId = openSessionParam(
    ...
    if (!isValidId(fileId)) {
      return res.status(400).json({ error: 'Invalid output file id' });
        ...
        const uploadByteLimit = Math.min(
    ...
    if (!parsedLength.ok) {
            return res
                .status(parsedLength.status)
                .json({ error: parsedLength.error });
    }
    ...
    if (!originalFilename) {
            return res
                .status(400)
                .json({ error: 'X-Original-Filename is required' });
    }
...
app.post('/package-transport', async (req, res) => {
    ...
    let reservedBytes = 0;
    let opened: OpenPackageTransportResponse | undefined;
    try {
        let envelope: PackageTransportEnvelope;
        try {
            envelope = await readPackageTransportEnvelope(req);
        } catch (error) {
        ...
        fields.contentType = Array.isArray(
        ...
        reservedBytes = await reserveEgressFetchBytes({
            grant,
            maxBytes:
                opened.declaredBytes && opened.declaredBytes > 0
                    ? opened.declaredBytes
                    : opened.target.policy.limits.maxResponseBytes,
            maxAggregateBytes:
                opened.target.policy.limits.maxAggregateBytesPerGrant,
        });
        if (
            opened.declaredBytes !== undefined &&
            opened.declaredBytes > reservedBytes
        ) {
            opened.close();
            throw new ExternalFetchError('FETCH_BUDGET_EXCEEDED');
        }

        res.status(opened.response.statusCode ?? 502);
        setPackageTransportResponseHeaders(opened.response.headers, res);
        res.setHeader(
            'X-CodeAPI-Network-Policy-Digest',
            grant.network_policy_digest ?? '',
        );
        res.setHeader(
            'Trailer',
            'X-CodeAPI-Egress-Outcome, X-CodeAPI-Egress-Requests, X-CodeAPI-Egress-Bytes',
        );
        res.setHeader('Transfer-Encoding', 'chunked');
        const responseBytes = await pipeHttpsPassthroughBody(
            opened,
            res,
            reservedBytes,
        );
        ...
        reservedBytes = 0;
        const finalLedger = await assertEgressGrantActive(grant);
        fields.responseBytes = responseBytes;
        fields.outcome = 'success';
        res.addTrailers({
            'X-CodeAPI-Egress-Outcome': 'OK',
            'X-CodeAPI-Egress-Requests': String(finalLedger.fetch_count),
            'X-CodeAPI-Egress-Bytes': String(finalLedger.fetched_bytes),
        });
        res.end();
        return res;
    } catch (error) {
        ...
        if (grant && reservedBytes > 0) {
            try {
                ...
                logger.error(
                    'Failed to release package transport byte reservation',
                    {
                        grantHash: hashLabel(grant.grant_id),
...
app.all('/package-transport', (req, res) => {
    req.resume();
    res.setHeader('Connection', 'close');
    return res.status(404).type('text/plain').send('not found');
});
...
app.post('/external-fetch', async (req, res) => {
  ...
  try {
    ...
    res.locals.egressAuditFields = fields;
        ...
        opened = await openExternalFetch({
            url,
            policy,
            fetchCount: ledger.fetch_count,
        });
    ...
    if (grant && reservedBytes > 0) {
      ...
      try {
        ...
        reservedBytes = 0;
                ...
                logger.error(
                    'Failed to settle external fetch byte reservation',
                    {
          grantHash: hashLabel(grant.grant_id),
          outcome: 'ledger_commit_failed',
                    },
  ...
  try {
    ...
    if (!executionId || !callId || !opaqueCallbackToken) {
       ...
       * fingerprint the real route by comparing response shapes. */
            logger.warn('/tool-call rejected: missing PTC headers', {
    ...
    const length = parsedLength.length;
        const callback = openPtcCallbackToken(
            opaqueCallbackToken,
            env.EGRESS_GRANT_SECRET,
        );
        setPtcAudit(res, {
            callbackExecId: callback.exec_id,
            requestExecId: executionId,
        });
    if (callback.exec_id !== executionId) {
            throw new EgressGrantError(
                'scope_mismatch',
                'PTC callback token execution does not match request',
            );
    }
    const body = await readRequestBody(req);
    if (callback.allowed_tool_names !== undefined) {
      ...
      const toolName = (parsed as { tool_name?: unknown }).tool_name;
            if (
                ...
                !callback.allowed_tool_names.includes(toolName)
            ) {
                throw new EgressGrantError(
                    'scope_mismatch',
                    'PTC tool is outside callback token scope',
                );
      }
    ...
    await recordEgressToolCall(callback.grant_id, executionId);
        const upstream = await fetch(
            forwardUrl(env.EGRESS_GATEWAY_TOOL_CALL_SERVER_URL, '/tool-call'),
            {
      method: 'POST',
      headers: {
        ...injectTraceHeaders(),
...
if (process.env.CODEAPI_EGRESS_GATEWAY_AUTOSTART !== 'false') {
  server = app.listen(env.EGRESS_GATEWAY_PORT, () => {
        logger.info(
            `Egress gateway listening on port ${env.EGRESS_GATEWAY_PORT}`,
        );
  });
